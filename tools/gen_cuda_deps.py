...
INSTALLER_INDEXES_END = "# <<< END GENERATED CUDA INSTALLER INDEXES - DO NOT EDIT >>>"
...
def build_cuda_installer_fragment(config: CudaDepsConfig) -> CudaInstallerFragment:
    """Build generated shell arrays for each installer runtime extra."""
    lines = [
    ...
    for extra, urls in _installer_index_urls(config).items():
        ...
        if len(urls) == 1:
            ...
            continue
        lines.append(f"readonly -a {variable}=(")
        lines.extend(f"    {shlex.quote(url)}" for url in urls)
        lines.append(")")
    return CudaInstallerFragment(text="\n".join(lines))
...
def run_generation_command(
    ...
    """Load config and update or check generated dependency metadata."""
    config = load_cuda_deps_config(config_path)
    generated_pyproject = build_cuda_pyproject_fragment(config)
    if installer_path is None:
        return _update_pyproject(pyproject_path, check, generated_pyproject)

    generated_installer = build_cuda_installer_fragment(config)
    current_installer = installer_path.read_text(encoding="utf-8")
    apply_cuda_fragment_to_installer(current_installer, generated_installer)
...
def apply_cuda_fragment_to_installer(installer_text: str, generated: CudaInstallerFragment) -> str:
    """Return install_nss.sh with its generated runtime index block replaced."""
    begin_count = installer_text.count(INSTALLER_INDEXES_BEGIN)
    end_count = installer_text.count(INSTALLER_INDEXES_END)
    if begin_count == 0 or end_count == 0:
        raise ValueError("install_nss.sh: missing generated CUDA installer index markers")
    if begin_count != 1 or end_count != 1:
        raise ValueError("install_nss.sh must contain exactly one generated CUDA installer index block")
    begin = installer_text.index(INSTALLER_INDEXES_BEGIN)
    end = installer_text.index(INSTALLER_INDEXES_END)
    if end < begin:
        raise ValueError("install_nss.sh: generated CUDA installer index markers are reversed")
    content_start = begin + len(INSTALLER_INDEXES_BEGIN)
    return f"{installer_text[:content_start]}\n{generated.text}\n{installer_text[end:]}"
...
def _update_installer(
    installer_path: Path,
    check: bool,
    generated: CudaInstallerFragment,
) -> GenerationResult:
    current = installer_path.read_text(encoding="utf-8")
...
def _cli(
    ...
    try:
        result = run_generation_command(config, pyproject, check, installer)
